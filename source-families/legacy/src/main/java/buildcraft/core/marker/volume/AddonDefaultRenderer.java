...
public class AddonDefaultRenderer<T extends Addon> implements IFastAddonRenderer<T> {
    ...
    public AddonDefaultRenderer() {
        //? if <1.20 {
        s = Minecraft.getInstance().getModelManager().getAtlas(InventoryMenu.BLOCK_ATLAS).getSprite(new ResourceLocation("minecraft", "block/quartz_block_top")); // Quartz top is the neutral white fallback sprite for debug geometry.
        //?} else {
        /*?
        s = Minecraft.getInstance().getModelManager().getAtlas(InventoryMenu.BLOCK_ATLAS).getSprite(new ResourceLocation("minecraft", "block/quartz_block_top")); // Quartz top is the neutral white fallback sprite for debug geometry.
        ?*/
