...
public enum DebugRenderHelper implements IDetachedRenderer {
    ...
    public static void renderAABB(PoseStack pose, Matrix4f matrix, VertexConsumer bb, AABB aabb, int colour) {
            if (s == null) s = Minecraft.getInstance().getModelManager().getAtlas(InventoryMenu.BLOCK_ATLAS).getSprite(new ResourceLocation("minecraft", "block/quartz_block_top")); // Quartz top is the neutral white fallback sprite for debug geometry.
    	pose.pushPose();
