...
/*        renderPosition(pose, bb);
        ...
        VertexConsumer vertex = bb.addVertex(pose, position_x, position_y, position_z)
        .setColor(colour_r, colour_g, colour_b, colour_a)
        .setUv(tex_u, tex_v)
        .setOverlay(overlay)
        .setUv2(light_block << 4, light_sky << 4);
        setTransformedNormal(vertex, normalMatrix, normal_x, normal_y, normal_z);
    ...
    public void renderPosition(Matrix4f pose, VertexConsumer bb) {
        bb.addVertex(pose, position_x, position_y, position_z);
    }

    public void renderNormal(Matrix3f normalMatrix, VertexConsumer bb) {
        setTransformedNormal(bb, normalMatrix, normal_x, normal_y, normal_z);
    }

    private static void setTransformedNormal(VertexConsumer consumer, Matrix3f matrix, float x, float y, float z) {
        consumer.setNormal(
    ...
    public void renderTex(VertexConsumer bb, ISprite sprite) {
        bb.setUv(sprite.getInterpU(tex_u), sprite.getInterpV(tex_v));
    }
