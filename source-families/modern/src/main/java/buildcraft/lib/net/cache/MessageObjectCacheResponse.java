...
public class MessageObjectCacheResponse {
    ...
    public static final BiConsumer<MessageObjectCacheResponse, Supplier<BCPacketContext>> HANDLER = (message, ctx) -> {
        ...
        context.enqueueWork(() -> {
            try {
                ...
                if (connection == null) {
                    BCLog.logger.warn("Dropped object cache response without an active client connection");
                    return;
                }
                ...
                for (int i = 0; i < message.ids.length; i++) {
                    ...
                    RegistryFriendlyByteBuf payloadBuffer =
                        new RegistryFriendlyByteBuf(Unpooled.copiedBuffer(payload), connection.registryAccess());
                    try {
                        cache.readObjectClient(id, payloadBuffer);
                    } finally {
