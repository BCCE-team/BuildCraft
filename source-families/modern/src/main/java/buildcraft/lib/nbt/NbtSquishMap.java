...
public class NbtSquishMap {
    ...
    public void addTag(Tag nbt) {
        if (nbt instanceof StringTag) {
            String val = NbtCompat.getString((StringTag) nbt);
            if (!strings.contains(val)) {
                strings.add(val);
            }
        } else if (nbt instanceof ByteTag) {
            byte val = NbtCompat.getByte((net.minecraft.nbt.NumericTag) nbt);
            if (!bytes.contains(val)) {
                bytes.add(val);
            }
        } else if (nbt instanceof ShortTag) {
            short val = NbtCompat.getShort((ShortTag) nbt);
            if (!shorts.contains(val)) {
                shorts.add(val);
            }
        } else if (nbt instanceof IntTag) {
            int val = NbtCompat.getInt((IntTag) nbt);
            if (!ints.contains(val)) {
                ints.add(val);
            }
        } else if (nbt instanceof LongTag) {
            long val = NbtCompat.getLong((LongTag) nbt);
            if (!longs.contains(val)) {
                longs.add(val);
            }
        } else if (nbt instanceof FloatTag) {
            float val = NbtCompat.getFloat((FloatTag) nbt);
            if (!floats.contains(val)) {
                floats.add(val);
            }
        } else if (nbt instanceof DoubleTag) {
            double val = NbtCompat.getDouble((DoubleTag) nbt);
            ...
            if (!complex.contains(compound)) {
                for (String key : NbtCompat.getAllKeys(compound)) {
                    if (!strings.contains(key)) {
                        strings.add(key);
                    }
                    Tag child = NbtCompat.get(compound, key);
                    if (child != null) addTag(child);
                }
    ...
    public int indexOfTag(Tag nbt) {
        ...
        if (nbt instanceof ByteTag) {
            return bytes.indexOf(NbtCompat.getByte((net.minecraft.nbt.NumericTag) nbt));
        } else {
            offset += bytes.size();
        }
        if (nbt instanceof ShortTag) {
            return offset + shorts.indexOf(NbtCompat.getShort((ShortTag) nbt));
        } else {
            offset += shorts.size();
        }
        if (nbt instanceof IntTag) {
            return offset + ints.indexOf(NbtCompat.getInt((IntTag) nbt));
        } else {
            offset += ints.size();
        }
        if (nbt instanceof LongTag) {
            return offset + longs.indexOf(NbtCompat.getLong((LongTag) nbt));
        } else {
            offset += longs.size();
        }
        if (nbt instanceof FloatTag) {
            return offset + floats.indexOf(NbtCompat.getFloat((FloatTag) nbt));
        } else {
            offset += floats.size();
        }
        if (nbt instanceof DoubleTag) {
            return offset + doubles.indexOf(NbtCompat.getDouble((DoubleTag) nbt));
        } else {
            offset += doubles.size();
        }
        ...
        if (nbt instanceof StringTag) {
            return offset + strings.indexOf(NbtCompat.getString((StringTag) nbt));
        } else {
            offset += strings.size();
        }
