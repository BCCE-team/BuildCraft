	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick, AbstractContainerScreen<?> screen) {
		...
		if (this.fluidCache != null && amount > 0 && capacity > 0) {
			...
			FluidRenderer.drawFluidForGui(this.fluidCache, leftpos + x, toppos + y + ys,
				leftpos + x + xs, toppos + y + ys - filled, guiGraphics);
		}
	}

	public void postRender(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick, AbstractContainerScreen<?> screen) {
		if(tankx>=0&&tanky>=0)
			if (screen instanceof ContainerScreenBase<?> base && base.getBaseTexture() != null) {
                RenderCompat.blit(guiGraphics, base.getBaseTexture(), screen.getGuiLeft() + x, screen.getGuiTop() + y + 1, tankx, tanky, xs, ys);
            }
	}

	public void renderTooltip(GuiGraphics guiGraphics, int x, int y) {
		if (super.isHovering(x - screen.getGuiLeft(), y - screen.getGuiTop())) {
			RenderCompat.renderComponentTooltip(guiGraphics, screen.getMinecraft().font,
