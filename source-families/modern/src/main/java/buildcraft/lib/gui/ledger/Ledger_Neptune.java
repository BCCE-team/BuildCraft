...
public class Ledger_Neptune implements IInteractionElement, IContainingElement {
    ...
    public void drawBackground(GuiGraphics guiGraphics, float partialTicks) {
        ...
        split.draw(guiGraphics, startX, startY, interpWidth, interpHeight, colour);
        ...
        guiGraphics.enableScissor((int) Math.floor(pos2.getX()), (int) Math.floor(pos2.getY()),
            (int) Math.ceil(pos2.getX() + interpWidth - 4), (int) Math.ceil(pos2.getY() + interpHeight - 8));
        try {

            for (IGuiElement element : closedElements) {
                element.drawBackground(guiGraphics, partialTicks);
            }
            if (shouldDrawOpen()) {
                for (IGuiElement element : openElements) {
                    element.drawBackground(guiGraphics, partialTicks);
    ...
    public void drawForeground(GuiGraphics guiGraphics, float partialTicks) {
        ...
        guiGraphics.enableScissor((int) Math.floor(scissorX), (int) Math.floor(scissorY),
            (int) Math.ceil(scissorX + scissorWidth), (int) Math.ceil(scissorY + scissorHeight));
        try {
