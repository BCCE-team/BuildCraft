...
public class GuiElementStatementSource<S extends IGuiSlot> implements IInteractionElement {
    ...
    public void drawBackground(GuiGraphics guiGraphics, float partialTicks) {
        iterateSlots((s, area) -> {
            ...
            drawAt(guiGraphics, s, area.x, area.y);
        });
    }

    private void drawAt(GuiGraphics guiGraphics, @Nullable S slot, double x, double y) {
        drawGuiSlot(guiGraphics, slot, x, y);
    }

    public static void drawGuiSlot(GuiGraphics guiGraphics, @Nullable IGuiSlot guiSlot, double x, double y) {
        if (guiSlot instanceof IStatementParameter) {
            ParameterRenderer.draw(guiGraphics, (IStatementParameter) guiSlot, x, y);
            return;
        }
        ...
        if (guiSlot != null) {
            ...
            if (sprite != null) {
                GuiIcon.drawAt(guiGraphics, sprite, x + 1, y + 1, 16);
            }
        }
    }
