...
public class GateLogic implements IGate, IRedstoneStatementContainer, GateView, GateControl {
    ...
    public GateLogic(PluggableGate pluggable, CompoundTag nbt) {
        this(pluggable, new GateVariant(NbtCompat.getCompound(nbt, "variant")));
    ...
    public void readConfigData(CompoundTag nbt) {
        short c = NbtCompat.getShort(nbt, "connections");
        ...
        for (int i = 0; i < statements.length; i++) {
            ...
            // Legacy
            if (NbtCompat.contains(nbt, tName, Tag.TAG_STRING)) {
                CompoundTag nbt2 = new CompoundTag();
                nbt2.putString("kind", NbtCompat.getString(nbt, tName));
                nbt2.putByte("side", NbtCompat.getByte(nbt, tName + ".side"));
                nbt.put(tName, nbt2);
            }
            // Legacy
            if (NbtCompat.contains(nbt, aName, Tag.TAG_STRING)) {
                CompoundTag nbt2 = new CompoundTag();
                nbt2.putString("kind", NbtCompat.getString(nbt, aName));
                nbt2.putByte("side", NbtCompat.getByte(nbt, aName + ".side"));
                nbt.put(aName, nbt2);
            }

            statements[i].trigger.readFromNbt(NbtCompat.getCompound(nbt, tName));
            statements[i].action.readFromNbt(NbtCompat.getCompound(nbt, aName));
        }
    }
        ...
        for (StatementPair pair : statements) {
            ...
            buildcraft.api.v2.statement.StatementSlot apiTrigger = trigger == null ? null
                : new buildcraft.api.v2.statement.StatementSlot(
                    StatementKind.TRIGGER, java.util.Objects.requireNonNull(net.minecraft.resources.Identifier.tryParse(trigger.getUniqueTag())),
            ...
            buildcraft.api.v2.statement.StatementSlot apiAction = action == null ? null
                : new buildcraft.api.v2.statement.StatementSlot(
                    StatementKind.ACTION, java.util.Objects.requireNonNull(net.minecraft.resources.Identifier.tryParse(action.getUniqueTag())),
    ...
    public void resolveActions() {
        ...
        // A gate is ordered: when multiple active rows write the same setting, the last row wins. Apply only the
        // final value to avoid intermediate mutations/events (for example EAST immediately followed by WEST on a
        // directional pipe action). Different source parts remain independent targets.
