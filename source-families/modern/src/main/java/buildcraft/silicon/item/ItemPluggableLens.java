...
public class ItemPluggableLens extends Item implements IItemPluggable, ICreativeTabItemProvider {
...
/*	@Override
    ...
    public void addCreativeTabItems(Consumer<ItemStack> output) {
        for (int i = 0; i < 34; i++) {
            output.accept(getStack(new LensData(i)));
        }
    }
	...
	public static class LensData {
        ...
        public LensData(ItemStack stack) {
            this(readLegacyMetadata(stack));
        }

        private static int readLegacyMetadata(ItemStack stack) {
        ...
        public ItemStack writeToStack(ItemStack stack) {
            ...
            data.putInt("Damage", getItemDamage());
            ItemStackUtil.setCustomData(stack, data);
            // The lens is not damageable: minecraft:damage would be clamped/normalised and variants collapse.
