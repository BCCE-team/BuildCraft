...
public class ScreenReplacer extends AbstractContainerScreen<MenuReplacer> {
    ...
    private void renderBlueprintPreview(GuiGraphics guiGraphics) {
        ...
        if (previewStacks.isEmpty()) {
            renderPreviewMessage(guiGraphics, Component.translatable("gui.buildcraftbuilders.replacer.preview.empty"), TEXT_COLOUR);
            return;
        }
        ...
        if (previewStacks.size() > max) {
            ...
            guiGraphics.drawString(this.font, more, this.leftPos + PREVIEW_X + PREVIEW_W - this.font.width(more) - 4, this.topPos + PREVIEW_Y + PREVIEW_H - 23, TEXT_COLOUR, false);
        }
    }
    ...
    private void renderReplacementStatus(GuiGraphics guiGraphics) {
        ...
        guiGraphics.drawString(this.font, text, x, y, status.colour, false);
    }
