...
import buildcraft.transport.pipe.flow.PipeFlowForgeEnergy.Section;
...
public enum PipeFlowRendererFE implements IPipeFlowRenderer<PipeFlowForgeEnergy> {
    INSTANCE;

	public static TextureAtlasSprite FE_FLOW;
    ...
    @Override
	public void render(PipeFlowForgeEnergy flow, float partialTicks, PoseStack matrix, MultiBufferSource buffer,
    	...
    	if (FE_FLOW == null) {
            ...
            FE_FLOW = atlas.apply(ResourceLocation.parse("buildcrafttransport:pipes/fe_flow"));
    	}
        // Resource reloads must not turn an optional visual effect into a client crash.
        if (FE_FLOW == null) {
            return;
        }
    	Pose p = matrix.last();
    ...
    private static void renderSidePower(Direction side, double power, double centrePower, double offset, Matrix4f pose, Matrix3f normal,
        ...
        TextureAtlasSprite sprite = FE_FLOW;
