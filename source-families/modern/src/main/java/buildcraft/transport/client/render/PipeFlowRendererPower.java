...
public enum PipeFlowRendererPower implements IPipeFlowRenderer<PipeFlowPower> {
	...
	public void render(PipeFlowPower flow, float partialTicks, PoseStack matrix, MultiBufferSource buffer,
    	...
    	if(POWER_FLOW_OVERLOAD == null || POWER_FLOW == null) {
            ...
            POWER_FLOW_OVERLOAD = atlas.apply(ResourceLocation.parse("buildcrafttransport:pipes/power_flow_overload"));
            POWER_FLOW = atlas.apply(ResourceLocation.parse("buildcrafttransport:pipes/power_flow"));
    	}
        // Never let a missing/late atlas sprite take down gameplay. On 1.21.11 the
        // actual lookup is backed by RenderCompat and normally resolves to missingno;
        // this guard keeps all maintained NeoForge targets safe during resource reloads.
        if (POWER_FLOW == null) {
            return;
        }
        if (POWER_FLOW_OVERLOAD == null) {
            POWER_FLOW_OVERLOAD = POWER_FLOW;
        }
    	Pose p = matrix.last();
