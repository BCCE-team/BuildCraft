...
public class BoardRobotBuilder extends RedstoneBoardRobot {
    ...
    private List<ItemStack> getMissingRequirements(List<RobotBuildTask> tasks) {
        ...
        for (ItemStack requirement : required) {
            ...
            for (int slot = 0; slot < robot.getContainerSize() && left > 0; slot++) {
                ...
                if (!stack.isEmpty() && ItemStack.isSameItemSameComponents(stack, requirement)) {
    ...
    public void writeSelfToNBT(CompoundTag nbt) {
        ...
        if (!currentTasks.isEmpty()) {
            nbt.put("currentTasks", NBTUtilBC.writeObjectList(currentTasks.stream().map(task -> task.writeToNBT(robot.level().registryAccess()))));
        }
        if (requirementsToLookFor != null) {
            nbt.put("requirementsToLookFor", NBTUtilBC.writeObjectList(requirementsToLookFor.stream().map(stack -> ItemStackUtil.saveOptional(stack, robot.level().registryAccess()))));
        }
    }
    ...
    public void loadSelfFromNBT(CompoundTag nbt) {
        ...
        if (nbt.contains("builderPos")) {
            builderPos = RoboticsNbtUtil.readBlockPos(nbt, "builderPos");
        }
        currentTasks.clear();
        if (nbt.contains("currentTasks")) {
            NBTUtilBC.readCompoundList(nbt.get("currentTasks"))
                .map(tag -> new RobotBuildTask(tag, robot.level().registryAccess()))
                .forEach(currentTasks::add);
        } else if (nbt.contains("currentTask")) {
            currentTasks.add(new RobotBuildTask(nbt.getCompound("currentTask"), robot.level().registryAccess()));
        }
        if (nbt.contains("requirementsToLookFor")) {
            requirementsToLookFor = new LinkedList<>(
                NBTUtilBC.readCompoundList(nbt.get("requirementsToLookFor"))
                    .map(tag -> ItemStackUtil.parseOptional(robot.level().registryAccess(), tag))
                    .toList()
            );
        }
    }
