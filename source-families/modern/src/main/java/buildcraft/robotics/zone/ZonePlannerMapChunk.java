...
public class ZonePlannerMapChunk {
    ...
    public ZonePlannerMapChunk(Level world, ZonePlannerMapChunkKey key) {
        ...
        for (int localX = 0; localX < 16; localX++) {
            ...
            for (int localZ = 0; localZ < 16; localZ++) {
                ...
                int mapColour = current.mapColor.calculateARGBColor(brightness);
                data[localX][localZ] = new MapColourData(current.posY, mapColour);
            }
        }
    }
    ...
    private static SurfaceSample sampleSurface(Level world, LevelChunk chunk, int localX, int localZ,
            int baseX, int baseZ) {
        int maxY = LevelCompat.getMaxBuildHeight(world) - 1;
        int minY = LevelCompat.getMinBuildHeight(world);
