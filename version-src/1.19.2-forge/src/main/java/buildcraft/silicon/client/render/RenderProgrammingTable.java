...
public class RenderProgrammingTable implements BlockEntityRenderer<TileProgrammingTable_Neptune> {
    ...
    public void render(@Nonnull TileProgrammingTable_Neptune tile, float partialTicks, PoseStack matrix, MultiBufferSource buffer, int combinedLight, int overlay) {
        ...
        bb.vertex(4 / 16D, 9 / 16D, 4 / 16D).color(255, 255, 255, 255).uv(whiteStainedGlass.getU(4), whiteStainedGlass.getV(4)).overlayCoords(overlay).uv2(light1, light2).normal(normal, 0, 1, 0).endVertex();
        bb.vertex(4 / 16D, 9 / 16D, 12 / 16D).color(255, 255, 255, 255).uv(whiteStainedGlass.getU(4), whiteStainedGlass.getV(12)).overlayCoords(overlay).uv2(light1, light2).normal(normal, 0, 1, 0).endVertex();
        bb.vertex(12 / 16D, 9 / 16D, 12 / 16D).color(255, 255, 255, 255).uv(whiteStainedGlass.getU(12), whiteStainedGlass.getV(12)).overlayCoords(overlay).uv2(light1, light2).normal(normal, 0, 1, 0).endVertex();
        bb.vertex(12 / 16D, 9 / 16D, 4 / 16D).color(255, 255, 255, 255).uv(whiteStainedGlass.getU(12), whiteStainedGlass.getV(4)).overlayCoords(overlay).uv2(light1, light2).normal(normal, 0, 1, 0).endVertex();
