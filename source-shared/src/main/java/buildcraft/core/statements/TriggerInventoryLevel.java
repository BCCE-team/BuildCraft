...
public class TriggerInventoryLevel extends BCStatement implements ITriggerExternal {
    ...
    public boolean isTriggerActive(BlockEntity tile, Direction side, IStatementContainer container,
        ...
        ItemStorage itemHandler = PlatformStorage.items(tile.getLevel(), tile.getBlockPos(), side.getOpposite());
        if (itemHandler == null) {
            return false;
        }
        FilteredItemStorage filters = ObjectUtilBC.castOrNull(itemHandler, FilteredItemStorage.class);
